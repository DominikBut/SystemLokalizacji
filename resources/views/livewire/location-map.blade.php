                    ...
                    <dl class="space-y-4 text-sm lg:text-base leading-7 text-stone-700 w-[32rem]">
                          ...
                          <div class="bg-blue-100 rounded flex p-2 h-full items-center font-bold">
                              ...
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-linejoin="round" stroke-width="2" class="text-sky-950 w-6 h-6 flex-shrink-0 mr-4" stroke="currentColor" className="size-6">
                                ...
                                <span class="@if($this->lokacja->strength < 20) text-red-500 @elseif($this->lokacja->strength < 50) text-yellow-500 @else text-green-500 @endif">
                                    {!! (!empty($this->lokacja)) ? '&nbsp;'. $this->lokacja->strength.'%'  : 'Brak odebranych danych' !!}
                                </span>
                                {!! '&nbsp;&nbsp;&nbsp;&nbsp;' !!}
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-linejoin="round" stroke-width="2" class="text-sky-950 w-6 h-6 flex-shrink-0 mr-4" stroke="currentColor" className="size-6">
                                ...
                                Bateria:
                                <span class="@if($this->lokacja->battery == 0) @elseif($this->lokacja->battery < 20) text-red-500 @elseif($this->lokacja->battery < 50) text-yellow-500 @else text-green-500 @endif">
                                    @if(!empty($this->lokacja) and $this->lokacja->battery == 0)
                                    {!! '&nbsp;'. 'Brak zapisu' !!}
                                    @else
                                    {!! (!empty($this->lokacja)) ? '&nbsp;'. $this->lokacja->battery.'%'  : 'Brak odebranych danych' !!}
                                    @endif
                                </span>
                                @else
                                Brak odebranych danych
                      ...
                      <div class="relative">
                              <div class="bg-blue-100 rounded flex p-2 h-full items-center">
                                  ...
                                  <p class="font-bold">{{ !empty($this->lokacja) ? App\Models\Coordinates::formatCreatedAt($this->lokacja->created_at) : 'Brak odebranych danych' }} </p>
